...
export const ProductValidation = [
    body("name").trim().escape().isString().isLength({ min: 1}).withMessage("O nome do produto é obrigatório."),
    body("price").trim().escape().customSanitizer(value => {
        value = value.trim();
    ...
    body("description").trim().escape().isString().isLength({ min: 1}).withMessage("A descrição do produto é obrigatória."),
    body("category_id").trim().escape().isString().isLength({ min: 1}).withMessage("A categoria é obrigatória.")
]